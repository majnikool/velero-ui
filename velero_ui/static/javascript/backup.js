...
function fetchData() {
  fetch('backups')
    ...
    .then((data) => {
      const backupList = parseBackupData(JSON.parse(data));
...
function parseBackupData(data) {
  ...
  function parseBackup(backup) {
    ...
    const {
      ...
      spec: { storageLocation, labelSelector },
    } = backup;
...
async function createBackup() {
  ...
  const metadata_name = createBackupForm.metadata_name.value;
  const spec_includedNamespaces = createBackupForm.spec_includedNamespaces.value;
  const spec_ttl = createBackupForm.spec_ttl.value;
  const spec_labels = createBackupForm.spec_labels.value;

  if (!metadata_name || !spec_includedNamespaces || !spec_ttl) {
